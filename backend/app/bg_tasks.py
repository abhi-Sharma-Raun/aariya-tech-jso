...
async def prepare_reports(session_id: uuid.UUID):
    ...
    async with AsyncSessionLocal() as db_session:
        ...
        if interview_session is None or interview_audio is None:
            return
        ...
        transcript = models.Transcripts(
            id=transcript_id,
            session_id=session_id,
            transcript=reports["preprocessed_transcript"]
        )

        report_row = models.Reports(
            session_id=session_id,
            analysis_report=reports["analysis_report"],
        ...
        interview_session.is_flagged = False
